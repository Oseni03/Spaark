...
const metadataSchema = z
	.object({
		css: z
			.object({ value: z.string().optional(), visible: z.boolean() })
			.optional(),
...
export const reactiveResumeV3Schema = z.object({
	...
	sections: z.object({
		...
		awards: sectionSchema
			.extend({ items: z.array(awardSchema) })
			.optional(),
		skills: sectionSchema
			.extend({ items: z.array(skillSchema) })
			.optional(),
		projects: sectionSchema
			.extend({ items: z.array(projectSchema) })
			.optional(),
		education: sectionSchema
			.extend({ items: z.array(educationSchema) })
			.optional(),
		interests: sectionSchema
			.extend({ items: z.array(interestSchema) })
			.optional(),
		languages: sectionSchema
			.extend({ items: z.array(languageSchema) })
			.optional(),
		volunteer: sectionSchema
			.extend({ items: z.array(volunteerSchema) })
			.optional(),
		references: sectionSchema
			.extend({ items: z.array(referenceSchema) })
			.optional(),
		publications: sectionSchema
			.extend({ items: z.array(publicationSchema) })
			.optional(),
