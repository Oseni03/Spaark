...
const serializeDate = (date) => {
	...
	if (date instanceof Date) return format(parseISO(date), "MMMM dd, yyyy");
	return null;
};
...
export const transformPortfolio = (portfolio) => {
	...
	return {
		...
		metadata: { ...defaultMetadata, ...portfolio.metadata },
...
export const getSocialIcon = (social) => {
	if (social.network === "github") return <IconBrandGithubFilled size={4} />;
	else if (social.network === "linkedin")
		return <IconBrandLinkedinFilled size={4} />;
	else if (social.network === "x") return <IconBrandXFilled size={4} />;
	else if (social.network === "youtube")
		return <IconBrandYoutubeFilled size={4} />;
};
